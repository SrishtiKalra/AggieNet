            ...
            public void onClick(View view) {
                ...
                uploadTask.addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
                    ...
                    public void onFailure(@NonNull Exception e) {
                        Toast.makeText(PostActivity.this, "Unsuccessful Upload", Toast.LENGTH_LONG).show();
                    }
                }).addOnCompleteListener(new OnCompleteListener<UploadTask.TaskSnapshot>() {
                    @Override
                    public void onComplete(@NonNull Task<UploadTask.TaskSnapshot> task) {
                        DatabaseReference databaseReference = FirebaseDatabase.getInstance("https://aggienet-posts.firebaseio.com/").getReference("Posts");
                            ...
                            public void onSuccess(Uri uri) {
                                ...
                                databaseReference.child(uniquepostid).setValue(map).addOnSuccessListener(new OnSuccessListener<Void>() {
                                    ...
                                    public void onSuccess(Void unused) {
                                        final int[] currentPostCount = {0};
                                        DatabaseReference userRef = FirebaseDatabase.getInstance("https://aggienet-users.firebaseio.com/").getReference("Users")
                                                .child(FirebaseAuth.getInstance().getCurrentUser().getUid());
                                        userRef.child("noofposts").addListenerForSingleValueEvent(new ValueEventListener() {
                                            @Override
                                            public void onDataChange(@NonNull DataSnapshot snapshot) {
                                                if (snapshot.exists()) {
                                                    currentPostCount[0] = snapshot.getValue(Integer.class);
                                                    currentPostCount[0]++;
                                                ...
                                                // Update the database with the new post count
                                                userRef.child("noofposts").setValue(currentPostCount[0]).addOnSuccessListener(new OnSuccessListener<Void>(){
                                                    @Override
                                                    public void onSuccess(Void unused) {
                                                        // The post count has been updated successfully
                                                    ...
                                                    @Override
                                                    public void onFailure(@NonNull Exception e) {
                                                        Toast.makeText(PostActivity.this, "Post unsuccessful", Toast.LENGTH_LONG).show();
                                                    }
                                                });
                                                }
